import { GENERATE_CUCUMBER_HTML_REPORT, GENERATE_CUCUMBER_JUNIT_REPORT, TEST_FAIL_FILE } from '../environment';
import { exec } from 'child_process';
import { writeFileSync } from 'fs';
import fetch from 'node-fetch';

export const isLiveModeOn = (): boolean => process.env.LIVE_MODE === 'off';
...
export const fetchAndAddVersionsToMetadata = () => {
  ...
  getVersion(`1.0.0/version`).then((res: any) => addMetadata('Some System', res.version));
};
...
export const generateJunitReport = () => {
  if (GENERATE_CUCUMBER_JUNIT_REPORT) {
    try {
      ...
      return exec(`node cucumber-junit.config.js`);
    } catch (error) {
